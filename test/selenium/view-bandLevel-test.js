...
describe('UI Testing for Band Level', () => {
  ...
  it(
    'Should display a table showing all the band level associated with job role',
    async () => {
      var driver = new webdriver.Builder()
        .withCapabilities(webdriver.Capabilities.chrome())
        .build()
      ...
      // Compare expected role name with actual role name for the first entry
      expect(
        await driver.findElement(By.id('jobSpecRoleID-1')).getText()).to.equal(
        'Data Analyst')
      // Compare expected band level with actual band level for the first entry

      expect(await driver.findElement(By.id('bandID-1')).getText()).to.equal(
        'Associate')
