...
describe('UI Testing for the matrix of roles page', () => {
    it('Should display the title from matrix of roles webpage', async() => {
        ...
        expect(await driver.findElement(By.id('matrixOfRolesTitle')).getText()).to.equal('Matrix of Roles');

        // Close browser
        await driver.quit();
    })

    it('Should display a matrix of roles for the Data & AI capability', async() => {
       var driver = new webdriver.Builder()
           .withCapabilities(webdriver.Capabilities.chrome())
           .build();
       ...
       expect(await driver.findElement(By.id('table-cell-1')).getText()).to.equal('Principle Data Architect');

       // Close browser
       await driver.quit();
    })

    it('Should display a matrix of roles for the Engineering capability', async() => {
       var driver = new webdriver.Builder()
           .withCapabilities(webdriver.Capabilities.chrome())
           .build();
       ...
       // Compare expected job role with its band level from the Data & AI capability
       expect(await driver.findElement(By.id('table-cell-1')).getText()).to.equal('Principle Architect');
    ...
    it('redirect to job specification when user click on the job roles display in the Data & AI capability matrix of roles', async() => {
       ...
       expect(await driver.findElement(By.linkText('Principle Data Architect')).click());
       // Compare expected title with actual title
    ...
    it('redirect to job specification when user click on the job roles display in the Engineering capability matrix of roles', async() => {
       ...
       expect(await driver.findElement(By.linkText('Principle Architect')).click());
       // Compare expected title with actual title
       expect(await driver.findElement(By.id('jobSpecificationTitle')).getText()).to.equal('Job Specification');
