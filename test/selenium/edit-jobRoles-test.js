...
/* globals describe, it */
describe(
  'UI Testing for Job Roles Webpage with added functionality (editing job roles row)',
  () => {
    it('Should allow a user to edit a job role of their choosing', async () => {
      ...
      expect(
        await driver.findElement(By.id('capabilityID-1')).getText()).to.equal(
        'Engineering')
    ...
    it(
      'Data Analyst should now be displayed in the Engineering Role of Matrix page rather than AI & Data',
      async () => {
        var driver = new webdriver.Builder()
          .withCapabilities(webdriver.Capabilities.chrome())
          .build()
        ...
        expect(await driver.findElement(
          By.linkText('Data Analyst')).getText()).to.equal('Data Analyst')
