...
describe('UI Testing for Competencies via bandID', () => {
   ...
   it('Should display a table with no data for competencies table with bandID 1 but contain training course', async() => {
        var driver = new webdriver.Builder()
            .withCapabilities(webdriver.Capabilities.chrome())
            .build();
  ...
  it('Should display a table with data for competencies table with bandID 4 and contain training course', async() => {
       var driver = new webdriver.Builder()
           .withCapabilities(webdriver.Capabilities.chrome())
           .build();
       ...
       expect(await driver.findElement(By.id('Competencies-Working within teams')).getText()).to.equal('Working within teams');
       // Compare expected training course name with actual training course name for bandID 4
       expect(await driver.findElement(By.id('Training-2022-10-04')).getText()).to.equal('Conducting Effective Meetings');
