...
describe('JobService', function () {
  ...
  describe('getJobRolesByCapability', function () {
    it('should return job roles from response', async () => {
      ...
      const data = [role]
      mock.onGet(JobService.URL).reply(200, data)
      var results = await apiInterface.getJobRolesByCapability(1)
      expect(results[0]).to.deep.equal(role)
    })
    ...
    it('should throw exception when 500 error returned from axios',
      async () => {
        ...
        try {
          ...
          var error = await apiInterface.getJobRolesByCapability(-1)
...
describe('addJobRole', function () {
  ...
  it('should return error message when error 400 occurs', async () => {
    ...
    try {
      var error = await apiInterface.addJobRole(newRole)
    } catch (e) {
      expect(e.message).to.equal('Bad request')
    }
  })
